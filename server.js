...
- Identify suspicious connections, payloads, or anomalies.
- Extract IOCs such as IPs, domains, ports, protocols, or file transfers.
...
app.post('/api/upload-protected', uploadProtected.any(), (req, res) => {
    if (!req.files || req.files.length === 0) {
        return res.status(400).json({ error: 'No files uploaded' });
    }
...
app.post('/api/volatility/analyze', async (req, res) => {
    ...
    // Volatility3 directory
    const volatilityDir = 'C:\\FYP_Virtual_lab\\volatility3-develop\\volatility3-develop';

    // Build PowerShell command with proper escaping
    const command = `Set-Location -Path '${volatilityDir}' ; python.exe .\\vol.py -f '${filePath}' ${volatilityPlugin}`;

    try {
        const result = PowerShellExecutor.executeCommand(command);
        
        if (result.success) {
            // Send volatility output to Flowise for analysis
            let flowiseAnalysis = null;
            try {
                const flowRes = await queryFlowise({
                    question: `Analyze this volatility memory dump output:\n\n${result.output}`
                });
                flowiseAnalysis = extractFlowiseText(flowRes);
            } catch (flowiseError) {
                console.error('Flowise query failed:', flowiseError);
            }
            ...
            res.json({
                ...
                flowiseAnalysis: flowiseAnalysis
            });
        } else {
            res.json({
                success: false,
        ...
        res.status(500).json({ error: 'Failed to execute volatility command', details: error.message });
    }
});
...
The report must include the following sections:`;
            ...
            analysisPrompt = STATIC_SYSTEM_PROMPT;
        }
...
app.post('/api/auto-process-protected-files', (req, res) => {
    ...
    try {
        ...
        for (const filename of files) {
            ...
            if (ext === '.xlsx' || ext === '.xls' || ext === '.docx' || ext === '.doc') {
                hashCommand = `Set-Location -Path '${johnDir}' ; python office2john.py '${filePath}' 2>&1`;
                hashTool = 'office2john.py';
            } else if (ext === '.pdf') {
                hashCommand = `Set-Location -Path '${johnDir}' ; perl pdf2john.pl '${filePath}' 2>&1`;
            ...
            if ((!output.trim()) && hashTool === 'office2john.py') {
                ...
                const altCommand = `python.exe '${johnDir}\\office2john.py' '${filePath}' 2>&1`;
            ...
            // Crack password
            const crackCommand = `Set-Location -Path '${johnDir}' ; .\\john.exe --wordlist='${rockyouPath}' '${hashFilePath}'`;
            console.log(`Cracking password for ${filename} with command: ${crackCommand}`);
            const crackResult = PowerShellExecutor.executeCommand(crackCommand);
            console.log(`Crack result for ${filename}: Success=${crackResult.success}, Output length=${crackResult.output.length}`);

            // Show results
            const showCommand = `Set-Location -Path '${johnDir}' ; .\\john.exe --show '${hashFilePath}'`;
            console.log(`Showing results for ${filename} with command: ${showCommand}`);
            const showResult = PowerShellExecutor.executeCommand(showCommand);
            console.log(`Show result for ${filename}: Success=${showResult.success}, Output length=${showResult.output.length}`);
...
app.post('/api/generate-hash', async (req, res) => {
    ...
    if (ext === '.xlsx' || ext === '.xls' || ext === '.docx' || ext === '.doc') {
        ...
        command = `Set-Location -Path '${johnDir}' ; python office2john.py '${filePath}' 2>&1`;
        hashTool = 'office2john.py';
    } else if (ext === '.pdf') {
        hashFilename = `${path.basename(filename, path.extname(filename))}.hash`;
        hashFilePath = path.join(hashesDir, hashFilename);
        command = `Set-Location -Path '${johnDir}' ; perl pdf2john.pl '${filePath}' 2>&1`;
        hashTool = 'pdf2john.pl';
    } else {
...
app.post('/api/john/crack', (req, res) => {
    ...
    const command = `Set-Location -Path '${johnDir}' ; .\\john.exe --wordlist='${rockyouPath}' '${hashFilePath}'`;
...
app.post('/api/process-protected-files', async (req, res) => {
    ...
    for (const file of files) {
        ...
        if (ext === '.xlsx' || ext === '.xls' || ext === '.docx' || ext === '.doc') {
            hashCommand = `Set-Location -Path '${johnDir}' ; python office2john.py '${filePath}'`;
            hashTool = 'office2john.py';
        } else if (ext === '.pdf') {
            hashCommand = `Set-Location -Path '${johnDir}' ; perl pdf2john.pl '${filePath}'`;
            hashTool = 'pdf2john.pl';
        } else {
        ...
        try {
            ...
            results.push({
                ...
                hashTool,
                crackOutput: crackResult.output,
                message: 'Hash generated and cracking completed'
            });
        } catch (error) {
            results.push({ 
                filename, 
